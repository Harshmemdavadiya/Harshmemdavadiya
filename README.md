<h1 align="center">Hi 👋, I'm Harsh Memdavadiya</h1>
<h3 align="center">Java Backend Developer | Spring Boot Enthusiast | MCA Student</h3>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Poppins&weight=600&size=24&duration=3000&pause=1000&color=00C2FF&center=true&vCenter=true&width=600&lines=Java+Backend+Developer;Spring+Boot+Developer;JSP+%7C+Servlet+%7C+JDBC;Always+Learning+New+Technologies" />
</p>

<p align="center">
  <a href="https://github.com/Harshmemdavadiya">
    <img src="https://komarev.com/ghpvc/?username=Harshmemdavadiya&label=Profile%20Views&color=0e75b6&style=for-the-badge" />
  </a>
  <a href="https://github.com/Harshmemdavadiya?tab=followers">
    <img src="https://img.shields.io/github/followers/Harshmemdavadiya?label=Followers&style=for-the-badge" />
  </a>
  <a href="https://linkedin.com/in/harsh-memdavadiya-804257305">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-0077B5?style=for-the-badge&logo=linkedin" />
  </a>
</p>

---

### 💫 About Me

- 🎓 Pursuing **MCA** at Kadi Sarva Vishwavidyalaya (CGPA: 9.29)
- 💼 Former **Java Developer Intern** at KaiMani Systems
- 💻 Focused on **Java backend development** with Spring Boot
- 🌱 Currently deepening my skills in **Spring Boot, REST APIs, and Microservices**
- 🚀 Interested in designing and building scalable backend systems
- 📫 Reach me at **harshmali482@gmail.com**

---

### 💼 Experience

**Java Developer Intern — KaiMani Systems**
*Jan 2026 – Apr 2026*

- Built backend features in Java, applying core Spring Boot concepts
- Implemented CRUD operations across multiple modules
- Integrated and managed a PostgreSQL database
- Debugged issues and optimized application performance
- Collaborated with a team on real-world project delivery

---

### 🎓 Education

| Degree | Institution | Score |
|---|---|---|
| Master of Computer Applications (MCA) | Kadi Sarva Vishwavidyalaya | CGPA: 9.29 |
| Bachelor of Computer Applications (BCA) | Adarsh BCA College | 75.92% |

---

### 🚀 Featured Projects

**🍔 Food Ordering System**
`Java` `JSP` `Servlet` `JDBC` `MySQL` `Bootstrap`
User authentication, cart management, order placement, admin dashboard, and a responsive UI.

**🐾 PetCareHub**
`Java` `Spring Boot` `PostgreSQL`
CRUD operations, REST API fundamentals, and database integration.

**🛒 Super Mart Management System**
`ASP.NET` `HTML` `CSS` `JavaScript` `Bootstrap`
Product management, billing system, and inventory tracking.

**🎮 Tic Tac Toe Game**
`Java` `JSP` `Servlet` `HTML` `CSS` `JavaScript`
Two-player gameplay with winner and draw detection, built with a responsive interface.

---

### 💻 Tech Stack

**Languages**
<p><img src="https://skillicons.dev/icons?i=java" /></p>

**Backend**
<p><img src="https://skillicons.dev/icons?i=spring,maven" /></p>

**Frontend**
<p><img src="https://skillicons.dev/icons?i=html,css,js,bootstrap" /></p>

**Database**
<p><img src="https://skillicons.dev/icons?i=mysql,postgresql" /></p>

**Tools**
<p><img src="https://skillicons.dev/icons?i=git,github,idea,eclipse,vscode" /></p>

---

### 📊 GitHub Stats

![](https://github-readme-stats.vercel.app/api?username=Harshmemdavadiya&theme=tokyonight&show_icons=true&hide_border=true)

![](https://github-readme-streak-stats.herokuapp.com/?user=Harshmemdavadiya&theme=tokyonight&hide_border=true)

![](https://github-readme-stats.vercel.app/api/top-langs/?username=Harshmemdavadiya&layout=compact&theme=tokyonight&hide_border=true)

---

### 🏆 GitHub Trophies

![](https://github-profile-trophy.vercel.app/?username=Harshmemdavadiya&theme=tokyonight&no-frame=true&margin-w=15)

---

### 🌱 Currently Learning

- Spring Boot (Advanced)
- REST APIs
- Hibernate & JPA
- Microservices
- Docker
- Data Structures & Algorithms

---

## 🤝 Connect With Me

<p align="center">

<a href="https://www.linkedin.com/in/harsh-memdavadiya-804257305" target="_blank">
  <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
</a>

<a href="https://mail.google.com/mail/?view=cm&fs=1&to=harshmali482@gmail.com" target="_blank">
  <img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Gmail"/>
</a>

<a href="https://github.com/Harshmemdavadiya" target="_blank">
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"/>
</a>

</p>
----
> *"Code. Learn. Build. Repeat."* 🚀

<div align="center">

### ⭐ Thanks for visiting my profile — feel free to star my repositories!

</div>
